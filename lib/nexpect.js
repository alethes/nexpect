  ...
  return {
    ...
    getline: function (callback) {
      if (typeof callback !== 'function')
        onError(new Error('The getline callback must be a function.'), true);
      var _getline = function _getline (data, context) {
        return callback(data, chain(context));
      };

      _getline.shift = false;
      _getline.description = '[getline]';
      _getline.requiresInput = true;
      context.queue.push(_getline);
      return chain(context);
    },
    ...
    run: function (callback) {
      ...
      function validateFnType (currentFn) {
        ...
        else if (['_expect', '_getline', '_sendline', '_wait', '_sendEof'].indexOf(currentFn.name) === -1) {
          //
      ...
      function evalContext (data, name) {
        ...
        if (currentFn.name === '_expect') {
          ...
          // then evaluate the next function (in case it is a `_sendline` function).
          //
        ...
        else if (currentFn.name === '_getline') {
